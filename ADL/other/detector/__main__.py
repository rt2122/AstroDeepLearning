...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Script for detector on Planck data.",
    ...
    parser.add_argument(
        "out_path", metavar="<out_path>", help="Output directory for scans."
    )
    parser.add_argument(
        "--data_path",
        metavar="<data_path>",
    ...
    parser.add_argument(
        "--thr", metavar="<thr>", default="0.1", help="Threshold for 'cat'."
    )
    return parser
...
if __name__ == "__main__":
    ...
    if args.command == "scan":
        scan_Planck_Unet(
            args.in_path, args.data_path, args.out_path, args.step, args.device
        )
