...
def radec2pix(ra: float, dec: float, nside: int, nest: bool = True) -> np.ndarray:
    ...
    """
    sc = SkyCoord(
        ra=np.array(ra) * u.degree, dec=np.array(dec) * u.degree, frame="icrs"
    )
    return hp.ang2pix(
        nside, sc.galactic.l.degree, sc.galactic.b.degree, nest=nest, lonlat=True
    )


def pix2radec(
    ipix: int, nside: int, nest: bool = True
...
def draw_circles(
    ras: np.ndarray,
    decs: np.ndarray,
    radiuses: Union[np.ndarray, float],
    nside: int,
    pix_matr: np.ndarray,
    centers_in_patch: bool = False,
) -> np.ndarray:
...
def draw_dots(
    ras: np.ndarray, decs: np.ndarray, nside: int, pix_matr: np.ndarray
) -> np.ndarray:
...
def src_on_batch(
    patch_line: Dict, f_matr: np.ndarray, cats: Dict[str, pd.DataFrame]
...
def draw_masks_and_save(
    cluster_cats_path: str,
    outpath: str,
    o_nside: int = 2,
    nside: int = 2**11,
    radius: float = 5 / 60,
    ...
    cluster_cats = {
        name: cat for name, cat in cats.items() if not "not_cluster" in name
    }
    non_cluster_cats = {
        name: cat for name, cat in cats.items() if "not_cluster" in name
    }
...
def calculate_n_src(
    ...
    for i in tqdm(range(hp.nside2npix(2))):
        ...
        pix_matr = one_pixel_fragmentation(o_nside, i, nside)
        pic = draw_dots(
            cluster_cat["RA"], cluster_cat["DEC"], nside=nside, pix_matr=pix_matr
        )
        n_src = []
...
def update_old_dataset(
    ...
    patch_coords = calculate_n_src(
        patch_coords, df, patch_size=patch_size, o_nside=o_nside, nside=nside
    )
...
def generate_patch_coords(
    ...
    n_patches: int = None,
    cats_subset: List[str] = None,
    fit_pixels: Dict[str, List[int]] = None,
    density_cat_path: str = None,
    patch_size: int = 64,
) -> pd.DataFrame:
    ...
    :rtype: pd.DataFrame
    """
    all_idx = generate_all_patches(
        cats_path, cats_subset=cats_subset, patch_size=patch_size
    )
    if n_patches is not None and len(all_idx) > n_patches:
        if fit_pixels is None:
            ...
            train_patches = all_idx[
                np.in1d(all_idx["pix2"], fit_pixels["train"])
            ].copy()
            ...
            if type(fit_pixels["example"]) == list:
                example_patches = all_idx[
                    np.in1d(all_idx["pix2"], fit_pixels["example"])
                ]
                train_patches = fit_patches_to_distribution(
                    example_patches, train_patches, n_train
                )
                val_patches = fit_patches_to_distribution(
                    example_patches, val_patches, n_val
                )
...
def fit_patches_to_distribution(
    example: pd.DataFrame, unfitted: pd.DataFrame, n_patches: int
) -> pd.DataFrame:
